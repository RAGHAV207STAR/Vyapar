...
export const generatePdfReport = async (data: PdfReportData) => {
  ...
  const applyHeaderFooter = (title: string, showCompany: boolean = true) => {
    ...
    if (showCompany) {
      ...
      doc.text(profile?.shopName || profile?.businessName || 'SMART VYAPAR', marginX, 15);
    ...
    doc.text(`Generated by Smart Vyapar • ${currentDateTime}`, marginX, A4_HEIGHT - 12);
  ...
  doc.setFontSize(8.5);
  doc.setTextColor(100, 116, 139); // slate-500
  doc.text('— Thank you for trusting Smart Vyapar —', A4_WIDTH / 2, footerY - 5, { align: 'center' });
  ...
  doc.text('SMART VYAPAR ANALYTICS ENGINE', 26, footerY + 5);
